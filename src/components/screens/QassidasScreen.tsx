...
const QassidasScreen = ({ initialQassidaId }: QassidasScreenProps) => {
  ...
  // Vérifier sessionStorage pour la navigation par titre
  useEffect(() => {
    if (allQassidas.length === 0 || hasNavigatedToInitial.current) return;

    const pendingTitle = sessionStorage.getItem('pendingXassidaTitle');
    if (pendingTitle) {
      const target = allQassidas.find((q) => q.title === pendingTitle);
      if (target) {
        hasNavigatedToInitial.current = true;
        handleQassidasClick(target);
        sessionStorage.removeItem('pendingXassidaTitle');
      }
    }
  }, [allQassidas, handleQassidasClick]);
  ...
  useEffect(() => {
    if (!initialQassidaId || allQassidas.length === 0 || hasNavigatedToInitial.current) return;

    const target = allQassidas.find((q) => Number(q.id) === Number(initialQassidaId));
    if (target) {
      hasNavigatedToInitial.current = true;
      handleQassidasClick(target);
    }
  }, [initialQassidaId, allQassidas, handleQassidasClick]);

  // Reset navigation flag when screen is closed
  useEffect(() => {
    if (!selectedQassida) {
